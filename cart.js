    ...
    const renderCartItem = (item) => `
        <div class="kwilt-cart-item" data-item-id="${item.item_id}">
            ...
            <div class="kwilt-cart-item-details">
                ...
                <p class="kwilt-cart-item-plan"><span class="kwilt-cart-item-plan-name" > ${item.plan_name.split(' ').join('-')} </span> Subscription - $${parseFloat(item.sku === "MEGA-MEMBERSHIP" ? item.row_total:item.monthly_price ).toFixed(0)}/${item.sku === "MEGA-MEMBERSHIP" ? 'year':'mo'}</p>
                ${item.sku === "MEGA-MEMBERSHIP" ?"": `<p class="kwilt-cart-item-supply">$${parseFloat(item.row_total).toFixed(0)} for a <span class="kwilt-cart-item-plan-name" > ${item.plan_name.split(' ').join('-')} </span> ${item.sku === "MEGA-MEMBERSHIP" ?'year':'supply'}</p>`}
